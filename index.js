...
/*🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀:
  ...
  When a variable is trying to be accessed but is not available in the local scope JavaScript will look a level up higher in the lexical or execution context to see if the variable is defined there. It will keep checking until it gets to the global scope. If it's never found then the JS knows the variable is undefined. In this case, the variable `internal` and its definition can be found in myFunction()'s execution context which is just one level up.
...
function animalNames(zooAnimalsArray) {
	...
	zooAnimalsArray.forEach(({ animal_name, scientific_name }) => {
...
function lowerCaseNames(zooAnimalsArray) {
	return zooAnimalsArray.map(({ animal_name }) => animal_name.toLowerCase());
}
...
function lowPopulationAnimals(zooAnimalsArray) {
	return zooAnimalsArray.filter(({ population }) => population < 5);
}
...
function USApop(zooAnimalsArray) {
	return zooAnimalsArray.reduce((popAccumulator, { population }) => {
		return popAccumulator + population;
	}, 0);
}
...
// Already used arrow functions in the MVP.
...
// const testCube = new CubeMaker({ length: 5, width: 5, height: 5 });

// console.log('testCube.volume: ', testCube.volume());
// console.log('testCube.surfaceArea: ', testCube.surfaceArea());
